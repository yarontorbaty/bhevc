...
int64_t av1_rd_pick_intra_sby_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  if (cpi->oxcf.q_cfg.aq_mode == EDGE_AQ && bsize >= BLOCK_8X8) {
    const int mi_row = xd->mi_row;
    const int mi_col = xd->mi_col;
    const int ori = av1_edge_block_orientation(cpi, x, bsize, mi_row, mi_col);
    if (ori >= 0) {
      /*
       * Map gradient orientation bin to the AV1 directional mode whose
       * prediction direction is perpendicular to the gradient (i.e.,
       * parallel to the edge).
       *
       * Gradient bin angles (center of 45-deg sector):
       *   0=0°, 1=45°, 2=90°, 3=135°, 4=180°, 5=225°, 6=270°, 7=315°
       *
       * The perpendicular edge angle = gradient_angle + 90°.
       * Bins wrap mod 8, and opposite bins (0/4, 1/5, etc.) map to the
       * same edge direction since edges are undirected.
       */
      static const PREDICTION_MODE ori_to_mode[NUM_ORI_BINS] = {
        V_PRED,    D67_PRED,  H_PRED,    D203_PRED,
        V_PRED,    D67_PRED,  H_PRED,    D203_PRED
      };
      PREDICTION_MODE best_dir = ori_to_mode[ori];

      static const int mode_angle[] = {
        [V_PRED] = 90, [H_PRED] = 180, [D45_PRED] = 45,
        [D135_PRED] = 135, [D113_PRED] = 113, [D157_PRED] = 157,
        [D203_PRED] = 203, [D67_PRED] = 67
      };
      static const PREDICTION_MODE directional_modes[] = {
        V_PRED, H_PRED, D45_PRED, D135_PRED,
        D113_PRED, D157_PRED, D203_PRED, D67_PRED
      };
      const int best_angle = mode_angle[best_dir];
      for (int i = 0; i < 8; i++) {
        PREDICTION_MODE m = directional_modes[i];
        if (m == best_dir) continue;
        int diff = abs(mode_angle[m] - best_angle);
        if (diff > 180) diff = 360 - diff;
        if (diff > 45)
          directional_mode_skip_mask[m] = 1;
      }
    }
  }
#endif
